...
def debug_version_structure():
    ...
    # get_regulation_content のテスト
    print("\n\n=== get_regulation_content テスト（修正版） ===")
    for ver in [2, 4, 5]:
        print(f"\nバージョン {ver} を取得中...")
        content = db.get_regulation_content(company_id, regulation_id, version=ver)
        if content:
            print(f"  ✅ 取得成功: version_number = {content.get('version_number')}")
        else:
            print(f"  ❌ 取得失敗")
