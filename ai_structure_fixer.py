...
【最重要：正常な項番号は絶対に触らない】
以下のパターンは「正常」なので、何も検出せず、何も修正しないでください：
✓ 「1. 会社は...」（項番号の直後に半角スペース+テキスト）
✓ 「2. 前項の...」（項番号の直後に半角スペース+テキスト）
✓ 「3.\n労働時間は...」（項番号の直後に改行1つ+テキスト）
✓ 「第3条(タイトル)」（条文タイトル）
✓ 「第24条(1年単位の変形労働時間制)」（条文タイトル）

以下のパターンは「異常」なので修正が必要です：
✗ 「会社は速やかに7.セクハラとは」（文章の途中に項番号が挿入されている）
   → 「会社は速やかに\n7. セクハラとは」に修正
✗ 「3.\n\n\n前項の...」（項番号とテキストの間に2行以上の空行）
   → 「3.\n前項の...」に修正（空行を1行に）

【判断基準 - 必ず守ること】
1. 項番号の直後に半角スペースまたは改行1つでテキストが続いている → 正常、何もしない
2. 項番号が文章の途中（前後に文字がある）に挿入されている → 異常、改行を追加して分離
3. 項番号とテキストの間に2行以上の空行がある → 異常、1行に修正
